...
export class DockerComputer implements IComputer {
  private id: string;
  ...
  // Overload for creating from experiment and agent index
  static async create(
    experiment: ExperimentResource,
    agentIndex: number,
  ): Promise<Result<DockerComputer>>;
  static async create(
    computerId: string,
    imageName?: string,
  ): Promise<Result<DockerComputer>>;
  static async create(
    experimentOrId: ExperimentResource | string,
    agentIndexOrImageName?: number | string,
  ): Promise<Result<DockerComputer>> {
    // Handle overload: if first param is ExperimentResource
    ...
    const id = experimentOrId;
    const imageName = agentIndexOrImageName as string | undefined;
    try {
      ...
      return ok(new DockerComputer(id, container));
    } catch (error: any) {
      return err(
        "computer_run_error",
  ...
  static async findById(computerId: string): Promise<DockerComputer | null> {
    const name = containerName(computerId);
    try {
      const container = docker.getContainer(name);
      await container.inspect();
      return new DockerComputer(computerId, container);
    } catch (_err) {
      return null;
    }
  }
  ...
  static async ensure(
    ...
    const c = await DockerComputer.findById(computerId);
    if (c) {
      ...
      if (status !== "running" || currentImage !== expectedImage) {
        await c.terminate();
        return DockerComputer.create(computerId, imageName);
      }
      return ok(c);
    }
    return DockerComputer.create(computerId, imageName);
  ...
  static async stopByExperiment(
    ...
    const listRes = await DockerComputer.listComputerIds();
    if (listRes.isErr()) {
      return listRes;
    }
    ...
    for (const id of matchingIds) {
      const computer = await DockerComputer.findById(id);
      if (computer) {
  ...
  static async copyToComputer(
    ...
    const computer = await DockerComputer.findById(computerId);
    if (!computer) {
      return err("computer_run_error", `DockerComputer ${computerId} not found`);
    }
